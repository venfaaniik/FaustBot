...
https://gist.github.com/Ixyk-Wolf/77f4acda8621ce948c3f2211cbe586b0
"""
...
class Music(commands.Cog):
    ...
    @commands.command()
    async def playsound(self, ctx, *, query):
        """Plays a file from the local filesystem"""

        if not ctx.voice_state.voice:
            await ctx.invoke(self._join)

        source = discord.PCMVolumeTransformer(discord.FFmpegPCMAudio("assets/sounds/" + query + ".mp3"))
